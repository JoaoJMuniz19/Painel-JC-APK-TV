(function () {
  ...
  async function openBatch(type) {
    ...
    $("jc_act_title").textContent = `Ativador ${type} — geração e envio em lote`;
    $("jc_act_qty").value = 1;
    $("jc_act_phone").value = "";
  ...
  function ensureModeModal() {
    let modal = $("jc_activation_mode_modal");
    if (modal) return modal;
    modal = document.createElement("div");
    modal.id = "jc_activation_mode_modal";
    modal.className = "jc-act-modal";
    modal.innerHTML = `<div class="jc-act-picker-box"><div class="jc-act-picker-head"><h3>Escolha o tipo de envio</h3><button type="button" class="jc-act-x" data-mode-close>Fechar</button></div><div class="jc-mode-options"><button type="button" class="jc-mode-option" data-mode="real"><b>🔑 Código real</b><span>Se ainda não existir um código válido, o painel gera automaticamente e segue o fluxo atual.</span></button><button type="button" class="jc-mode-option" data-mode="test"><b>🧪 Modo teste</b><span>Usa o código fixo de teste, o download 3754570 e não gera nem consome código real.</span></button></div></div>`;
    document.body.appendChild(modal);
    return modal;
  }
  ...
  function chooseDeliveryMode() {
    return new Promise((resolve) => {
      const modal = ensureModeModal();
      const finish = (value) => { modal.classList.remove("show"); resolve(value); };
      modal.querySelector('[data-mode-close]').onclick = () => finish(null);
      modal.querySelectorAll('[data-mode]').forEach((button) => button.onclick = () => finish(button.dataset.mode));
  ...
  async function loadTestSettings(type) {
    const appKey = String(type) === '11' ? 'activator_11' : 'activator_16';
    ...
    return {
      ...
      downloadCode: String(data?.test_download_code || '3754570').replace(/\D/g, ''),
      apkUrl: String(data?.test_apk_url || ''),
  ...
  async function generateRealSingle(type) {
    ...
    const result = await api.generate(type, { client: state.client });
  ...
  async function openTestSingle(type) {
    ...
    state.batchType = type;
    state.testSettings = await loadTestSettings(type);
    ...
    state.generated = [{
      ...
      generated_at: new Date().toISOString(),
      record_scope: 'test',
      origin: String(type) === '11' ? '11-Ativador' : '16/VIP',
    ...
    $('jc_act_title').textContent = `Ativador ${type} — enviar teste de 3 dias`;
    $('jc_act_qty').value = 1;
  ...
  async function openSingle(type) {
    await enforceDeadlinesOnce();
    const mode = await chooseDeliveryMode();
    if (!mode) return;
    if (mode === 'test') {
      try { await openTestSingle(type); } catch (error) { A.toast(error.message || String(error), 'error'); }
      return;
    }
    state.deliveryMode = 'real'; state.testSettings = null;
    setQuantityMode(false);
    const globalLast = window.JC_LAST_ACTIVATION;
    let last = state.last[type] || (String(globalLast?.type || '') === String(type) ? globalLast : null) || readDomActivation(type);
    ensureModal(); state.batchType = type;
    state.client = last?.client || (last ? { id:last.client_id,full_name:last.client_name,whatsapp:last.whatsapp } : null);
    $('jc_act_title').textContent = `Ativador ${type} — preparar envio real`;
    $('jc_act_qty').value = 1; $('jc_act_client_label').value = last?.client_name || 'Escolha o cliente'; $('jc_act_phone').value = last?.whatsapp || '';
    await loadDeliveryOptions(type);
    if (!state.client) await chooseClient();
    if (!state.client) return;
    if (!last) {
      try { $('jc_act_result').textContent = 'Gerando o código real...'; last = await generateRealSingle(type); state.last[type] = last; }
  ...
  function batchCreditRule(type) {
    const profile = getApi()?.profile?.();
    if (String(profile?.account_type) !== "credits") return { charged: false, cost: 0, balance: Infinity, fn: null };
    const fn = (getApi()?.functions?.() || []).find((item) => item.id === `activator${type}.generate`);
  ...
  async function generateBatch() {
    ...
    for (let index = 0; index < rows.length; index += 1) {
      ...
      try {
        const result = await api.generate(state.batchType, { client: state.client });
        if (!result?.ok || !result.code) throw new Error("Código não retornado.");
  ...
  function buildMessage() {
    ...
    if (state.deliveryMode === 'test') {
      ...
      const fallback = ['Olá! 👋','','Seu teste da *JC APK TV* foi preparado com sucesso.','','🧪 *TESTE GRÁTIS POR 3 DIAS*','','O teste está disponível somente para aparelhos que ainda não utilizaram o aplicativo UniTV Mensal. Caso o aplicativo já tenha sido utilizado anteriormente, será necessário adquirir uma licença de ativação permanente, sem mensalidades.','',`📦 *Produto:* Ativador ${state.batchType} Dígitos — Demonstração`,'','📺 *Aparelho 1*','• *Versão:* Tradicional',`• *Código de download — Opção 1:* \`${t.downloadCode || '3754570'}\``,'','🔗 *Link direto para instalação:*',t.apkUrl || '','', '▶️ *Tutorial de como utilizar:*',t.tutorialUrl || state.tutorialUrl || '','', '⏱️ *Duração do teste*','',`O próprio APK de demonstração controla automaticamente os ${t.durationDays || 3} dias. O teste não funcionará em aparelhos que já tenham utilizado o aplicativo UniTV Mensal.`,'','⚠️ *IMPORTANTE — TESTE GRATUITO*','','Não oferecemos suporte para instalação durante o período de teste gratuito. Siga corretamente o tutorial enviado acima.','','O suporte para instalação é disponibilizado somente após a compra do código de ativação.'];
      ...
      return template.replaceAll('{{TIPO_ATIVADOR}}',String(state.batchType)).replaceAll('{{CODIGO_TESTE}}',t.code||state.generated[0]?.code||'').replaceAll('{{CODIGO_DOWNLOAD}}',t.downloadCode||'3754570').replaceAll('{{LINK_VIDEO_TUTORIAL}}',t.tutorialUrl||state.tutorialUrl||'').replaceAll('{{DURACAO_DIAS}}',String(t.durationDays||3)).replaceAll('{{LINK_APK_TESTE}}',t.apkUrl||'');
    }
  ...
  async function logTestDelivery(phone, text) {
    ...
    const { error } = await A.client.from('jc_test_deliveries').insert({client_id:state.client?.id||null,actor_id:user?.id||null,activator_type:String(state.batchType),activator_origin:String(state.generated[0]?.origin||(String(state.batchType)==='11'?'11-Ativador':'16/VIP')),phone_full:phone,phone_masked:maskPhone(phone),test_code:t.code||state.generated[0]?.code||'',download_code:t.downloadCode||'3754570',apk_url:t.apkUrl||'',tutorial_url:t.tutorialUrl||state.tutorialUrl||'',message_text:text,duration_days:Number(t.durationDays||3),status:'whatsapp_opened',metadata:{source:'activation-delivery.js'}});
  ...
  async function init() {
    ...
    document.querySelectorAll("#digits_mass_panel [data-jc-batch]").forEach((button) => {
      button.onclick = () => openBatch(button.dataset.jcBatch === "16" ? "16" : "11");
    });
    ensureModal();
    bindCopyHistory();
  ...
  window.JC_ACTIVATION_DELIVERY = { openBatch, openSingle, buildMessage };
})();

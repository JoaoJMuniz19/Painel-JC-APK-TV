(function () {
  ...
  async function loadGeneral() {
    ...
    const defaultDay5BillingMessage = "Olá, {nome}. Tudo bem?\n\nEste é um lembrete da {empresa} referente à sua mensalidade do plano {plano}.\n\nMensalidade: {valor}\nVencimento: {vencimento}\nPrazo para pagamento sem bloqueio: até o dia 10\n{penalidade_bloco}\n{pix_bloco}\n\nApós realizar o pagamento, envie o comprovante por aqui para confirmarmos a renovação do seu acesso.\n\nObservação sobre controle de códigos: a cada 10 códigos gerados e não utilizados dentro do mesmo mês, poderá ser aplicado um acréscimo administrativo de 5% sobre a mensalidade. Quando houver acréscimo no período, ele será informado no resumo da cobrança com a quantidade e as datas dos códigos.\n\nSuporte via WhatsApp: {suporte}.";
    const defaultDay10BillingMessage = "Olá, {nome}. Tudo bem?\n\nHoje é o último dia para regularizar sua mensalidade da {empresa} antes do bloqueio temporário do acesso.\n\nMensalidade: {valor}\nVencimento: {vencimento}\nPrazo final para pagamento sem bloqueio: hoje, dia 10\n{penalidade_bloco}\n{pix_bloco}\n\nApós realizar o pagamento, envie o comprovante por aqui para confirmarmos a renovação do seu acesso.\n\nObservação sobre controle de códigos: a cada 10 códigos gerados e não utilizados dentro do mesmo mês, poderá ser aplicado um acréscimo administrativo de 5% sobre a mensalidade. Quando houver acréscimo no período, ele será informado no resumo da cobrança com a quantidade e as datas dos códigos.\n\nSuporte via WhatsApp: {suporte}.";
    const savedDay5 = state.billing.day5_message || "";
    const savedDay10 = state.billing.day10_message || "";
    const needsBillingUpgrade = (text) => !text || !String(text).includes("{penalidade_bloco}");
  ...
  function canonicalPermissionGroup(f) {
    ...
    if (id === "activator11.generate" || id === "activator16.generate" || id.startsWith("activators.mass") || gid === "activators_mass") {
      return { id: "activators_mass", name: "Dígitos em Massa" };
    }
    ...
    if (/(^|[._-])16($|[._-])/.test(id) || (/16/.test(raw) && activatorWords.test(raw))) {
      return { id: "activator16", name: "Ativador 16 dígitos" };
    }
    if (/(^|[._-])11($|[._-])/.test(id) || (/11/.test(raw) && activatorWords.test(raw))) {
      return { id: "activator11", name: "Ativador 11 dígitos" };
    }
  ...
  function orderedFunctionGroups(functions = state.functions) {
    const byId = new Map();
    (functions || []).filter((f) => String(f?.id || "") !== "activators.mass.open").forEach((f) => {
      const group = functionGroupForAdmin(f);
      const massName = String(f?.id || "") === "activator11.generate"
        ? "Liberar em massa — 11 dígitos"
        : String(f?.id || "") === "activator16.generate"
          ? "Liberar em massa — 16 dígitos"
          : f?.name;
      const item = { ...f, name: massName, group_id: group.id, group_name: group.name };
  ...
  function billingMessageForClient(c, type = "day5", penalty = null) {
    const pixLines=[];
    ...
    return String(billingTemplate(type)||"").replace(/\{(nome|usuario|plano|valor|vencimento|suporte|empresa|pix_nome|pix_banco|pix_tipo|pix_chave|pix_bloco|penalidade_bloco|penalidade_qtd|penalidade_percentual|penalidade_valor|total_cobranca|mes_referencia)\}/g,(_,key)=>vars[key]??"").replace(/\n{3,}/g,"\n\n").trim();
  }
  async function billingMessageForClientAsync(c, type = "day5") {
  ...
  function eligibleBillingClients(scope = "due") {
    ...
    return state.clients.filter((c)=>{
      ...
      if(billingStartsLater(c,today)||c.current_month_payment||c.next_paid_payment)return false;
      const expiry=dateOnly(c.expires_at);return Boolean(expiry&&expiry.slice(0,7)<=currentMonth);
    });
  }
  ...
  function openBillingClient(c) {
    ...
    billingMessageForClientAsync(c, "day5").then((message) => {
  ...
  function openPayment(c){
    ...
    const current=anchors[anchors.length-1]||today, next=A.addMonths(current,Number(c.plan_months)||1), grace=A.addDays(next,5);

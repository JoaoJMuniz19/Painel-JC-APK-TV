(function () {
  ...
  function expired(p) {
    ...
    if(type==="monthly"){
      const today=new Date().toISOString().slice(0,10),start=String(p.starts_at||"").slice(0,10),expiry=String(p.expires_at||"").slice(0,10),grace=String(p.grace_until||"").slice(0,10);
      if(start&&start>today)return false;
      if(expiry&&expiry>=today)return false;
      if(grace)return grace<today;
    }
    return false;
  }
  function store() {
  ...
  function entryScopeAllowed(f) {
    ...
    if (id === "activators.mass.open") {
      return Boolean(state.permissions?.["activator11.generate"] || state.permissions?.["activator16.generate"]);
    }
  ...
  function permissionCapture(e) {
    ...
    // O antigo aviso de 30 segundos foi preservado com a mesma mensagem,
    // mas agora usa o tempo de 10 segundos solicitado para 11 e 16 dígitos.
  ...
  function actuallyOwned(f){
    ...
    if(state.permissions?.[f.id])return true;
    if(String(f?.id||"")==="activators.mass.open")return Boolean(state.permissions?.["activator11.generate"]||state.permissions?.["activator16.generate"]);

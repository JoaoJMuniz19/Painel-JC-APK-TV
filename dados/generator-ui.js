(function () {
  ...
  function closeActivationSubmenus() {
    document.querySelectorAll(".submenu-gerar-combinacoes, .submenu-gerador11, .submenu-digits-mass").forEach((el) => setActive(el, false));
    document.querySelectorAll('[data-jc-action="open-config"], [data-jc-action="open-activator"], [data-jc-action="open-digits-mass"]').forEach((button) => button.setAttribute("aria-expanded", "false"));
    stopTimer("config");

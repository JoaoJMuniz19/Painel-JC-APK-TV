/*
  ...
  - Supabase, quando disponível, busca apenas configuração leve e links já cadastrados.
...
(function(){
  ...
  function updateVisibility(){
    uiInstances.forEach(function(ui){
      ...
      if(ui.type === 'embedded') ui.root.style.display = showPagePlayer ? '' : 'none';
    });
  }
  ...
  async function playMusic(){
    if(!remoteConfigLoaded) await fetchRemoteRows();
  ...
  function buildControls(root, type){
    ...
    root.innerHTML = '' +
      '<div class="jc-music-panel-head"><strong>🎵 Player de Música JC-APK TV</strong><small>Áudios no GitHub Releases · Supabase só configuração leve</small></div>' +

(function(){
  ...
  const state={context:null,packages:[],clients:[],balances:[],functions:[],sellableFunctions:[]};
  ...
  function injectStyles(){
    ...
    const st=document.createElement('style');st.id='jc_reseller_credit_styles';st.textContent=`
      ...
      @media(max-width:680px){.jcrc-head{align-items:flex-start;flex-direction:column}.jcrc-sim,.jcrc-grid{grid-template-columns:1fr}.jcrc-full{grid-column:auto}.jcrc-actions .jcrc-btn{width:100%}}
  ...
  function blockMarkup(){return `<section id="jc_reseller_credit_block" data-jc-function-id="reseller.credit.open" data-jc-function-category="Revenda por Créditos" data-jc-function-name="Revenda por Créditos" data-jc-action-kind="entry" data-jc-order="9999">
    <button class="activation-full-toggle jcrc-main-toggle" id="jcrc_toggle" type="button" aria-controls="jcrc_body" aria-expanded="true"><span>• ÁREA DO REVENDEDOR • <strong>Revenda por Créditos</strong></span><b id="jcrc_toggle_icon">▴</b></button>
    <div class="jcrc-body is-open" id="jcrc_body" aria-hidden="false">
  ...
  function bindCollapsible(toggle,body,icon){
    ...
    const setOpen=(open,announce)=>{body.classList.toggle('is-open',open);body.setAttribute('aria-hidden',open?'false':'true');toggle.setAttribute('aria-expanded',open?'true':'false');if(icon)icon.textContent=open?'▴':'▾';if(open&&announce)closeOtherMainSections();};
    toggle.addEventListener('click',()=>setOpen(!body.classList.contains('is-open'),true));
  ...
  function setupResellerCollapsibles(){
    const demoToggle=$('jc_reseller_credit_toggle'),demoBody=document.querySelector('#jc_reseller_credit_demo_card .jc-reseller-credit-body');
    bindCollapsible(demoToggle,demoBody,demoToggle?.querySelector('b'));
...
Depois que você enviar o cadastro, eu confiro no painel, libero as funções combinadas e coloco os créditos.`;
  ...
  async function activate(contextData){state.context=contextData||window.JC_GENERATOR_CONTEXT||{};const demo=$('jc_reseller_credit_demo_card');if(!permitted()){if(demo){demo.hidden=false;demo.style.display='';setupResellerCollapsibles();}return;}insertBlock();if(demo){demo.hidden=true;demo.style.display='none';}const real=$('jc_reseller_credit_block');if(real&&demo?.parentNode)demo.parentNode.insertBefore(real,demo.nextSibling);setupResellerCollapsibles();$('jcrc_lock').style.display='none';$('jcrc_real').classList.add('show');const admin=isAdmin();if($('jcrc_admin_clients_link'))$('jcrc_admin_clients_link').style.display=admin?'inline-flex':'none';refreshCadastroLink();$('jcrc_balances').innerHTML='<span class="jcrc-balance">Carregando saldo real da revenda...</span>';status('Revenda por créditos pronta: tela simplificada por produto/categoria, sem funções internas do ADM.');setTimeout(()=>loadRealData(false).catch(e=>status(e.message||'Não foi possível carregar a revenda.',true)),80);}
